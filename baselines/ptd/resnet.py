...
class ResNetInjected(nn.Module):
    def __init__(self, block, num_blocks, num_classes, conf):
        super(ResNetInjected, self).__init__()
        # self.in_planes = 64
    ...
    def forward(self, x, revision=True):
        ...
        _, logits, out_1 = self.backbone.forward_include_vector_before_last(x)
        out_2 = logits
        if revision == True:
...
def ResNet34(num_classes, conf):
    return ResNetInjected(BasicBlock, [3,4,6,3], num_classes, conf)
