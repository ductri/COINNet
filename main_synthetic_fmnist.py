...
parser.add_argument('--session_id',type=int,help='Session_ID',default=0)
...
parser.add_argument('--dataset',type=str,help='synthetic or cifar10 or mnist',default='fmnist')
...
parser.add_argument('--lam_trace',type=float,help='Volume regularizer',default=0.001)
...
parser.add_argument('--classifier_NN',type=str,help='resnet9 or resnet18 or resnet34',default='resnet18f')
...
parser.add_argument('--percent_instance_noise', type = float, help = 'percent of samples having instance-dependent noise', default =0.1)
...
parser.add_argument('--log_folder',type=str,help='log folder path',default='results/fmnist_synthetic/')
...
log_file_name = args.log_folder+'log_'+str(time_now.strftime("%b-%d-%Y"))+'_synthetic_fmnist_'+str(args.session_id)+'.txt'
result_file=args.log_folder+'result_'+str(time_now.strftime("%b-%d-%Y"))+'_synthetic_fmnist_'+str(args.session_id)+'.txt'
...
def main():
	...
	if args.flag_hyperparameter_tuning:
		if args.dataset=='cifar10':
			alg_options['lamda_list']=[0.01]
			alg_options['mu_list']=[0.01]
		else:
			alg_options['lamda_list']=[0.01]
			alg_options['mu_list']=[0.01]
	...
	for t in range(args.n_trials):
		...
		args.K = max(train_data.train_labels)+1
		print(args.K)
