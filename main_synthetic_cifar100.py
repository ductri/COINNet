...
parser.add_argument('--dataset',type=str,help='synthetic or cifar10 or mnist',default='cifar100')
...
log_file_name = args.log_folder+'log_'+str(time_now.strftime("%b-%d-%Y"))+'_synthetic_cifar100_'+str(args.session_id)+'.txt'
result_file=args.log_folder+'result_'+str(time_now.strftime("%b-%d-%Y"))+'_synthetic_cifar100_'+str(args.session_id)+'.txt'
...
def main():
	...
	for t in range(args.n_trials):
		...
		train_data 	= cifar100_dataset(True, transform=transform_train(args.dataset), target_transform=transform_target,split_per=0.95,args=args,logger=logger)
		val_data 	= cifar100_dataset(False, transform=transform_test(args.dataset), target_transform=transform_target,split_per=0.95,args=args,logger=logger)
		test_data 	= cifar100_test_dataset(transform=transform_test(args.dataset), target_transform=transform_target)
