...
from my_dataset import LitCIFAR10N


@hydra.main(version_base=None, config_path=f"conf", config_name="config_constraint")
def my_main(conf):
    ...
    run = wandb.init(entity='narutox', project=project_name, tags=['constraint', 'outlier', 'outlier_model'], config=OmegaConf.to_container(conf, resolve=True))
    args.dataset = conf.data.dataset

    args.instance_dep_percent_estim = conf.train.instance_dep_percent_estim
    ...
    def main():
        ...
        for t in range(args.n_trials):
            ...
            data_module = LitCIFAR10N(args.batch_size)
            data_module.setup('fit')
            data_module.setup('test')
            train_data = data_module.train_set
            val_data = data_module.val_set
            test_data = data_module.test_set
